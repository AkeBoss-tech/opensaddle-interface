...
test('a failure after offline install staging preserves the prior runtime', () => {
  ...
  const sourceArchive=path.join(inputs,'source.tar');assert.equal(spawnSync('tar',['-cf',sourceArchive,'-C',root,'scripts/build-krail-runtime.mjs']).status,0);const receipt=path.join(inputs,'source-receipt.json');writeFileSync(receipt,JSON.stringify({schemaVersion:1,repository:'https://github.com/AkeBoss-tech/opensaddle-interface.git',revision:'1'.repeat(40),tree:'4'.repeat(40),archiveSha256:digest(sourceArchive)}))
