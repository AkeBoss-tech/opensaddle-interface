...
test('PROJECT-AGENT-WRITE-REVIEW-1: desktop forwards only exact human review reads and digest-bound approval',async()=>{
 const base={expectedBaseUrl:handoff.baseUrl,expectedInstallationId:'install',expectedProjectId:'project'}
 const proposal='awp_'+'a'.repeat(32),digest='b'.repeat(64),seen:string[]=[]
 const server:typeof fetch=async(input,init)=>{const request=new Request(input,init);assert.equal(request.headers.get('Authorization'),'Bearer private-token');assert.equal(request.headers.get('X-OpenSaddle-User'),'owner');seen.push(request.method+' '+new URL(request.url).pathname);if(request.method==='POST')assert.deepEqual(JSON.parse(await request.text()),{expected_request_digest:digest});return Response.json({})}
 for(const path of ['/api/v2/runs/run-one/connector-write-proposals',`/api/v2/connector-write-proposals/${proposal}`])await proxyPersonalRuntimeRequest(handoff,{...base,method:'GET',path},server)
 await proxyPersonalRuntimeRequest(handoff,{...base,method:'POST',path:`/api/v2/connector-write-proposals/${proposal}/approve`,body:JSON.stringify({expected_request_digest:digest})},server)
 assert.equal(seen.length,3)
 for(const [method,path,body] of [
  ['POST',`/api/v2/connector-write-proposals/${proposal}/dispatch`,JSON.stringify({expected_request_digest:digest})],
  ['GET',`/api/v2/connector-write-proposals/${proposal}/approve`,undefined],
  ['POST',`/api/v2/connector-write-proposals/${proposal}/approve`,JSON.stringify({expected_request_digest:'bad'})],
  ['POST',`/api/v2/connector-write-proposals/${proposal}/approve`,JSON.stringify({expected_request_digest:digest,force:true})],
  ['POST',`/api/v2/connector-write-proposals/${proposal}/approve`,'not json'],
  ['GET','/api/v2/runs/run-one/connector-write-proposals?all=true',undefined],
