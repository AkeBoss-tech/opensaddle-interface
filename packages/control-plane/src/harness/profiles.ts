...
export const BUILTIN_PROFILES: HarnessProfile[] = [
  ...
  },
  {
    ...
    // Tool-level permission prompts are relayed through OpenSaddle's local MCP
    // bridge instead of requiring a coarse approval before Claude starts.
    approvalPolicy: 'none',
    codingAffinity: 0.92,
    supportsCancel: true,
    supportsStreaming: true,
  },
  {
    ...
    description: 'Cursor Agent CLI with NDJSON transcript and tool events.',
    kind: 'cli',
    promptMode: 'final_arg',
    baseArgs: ['--print', '--output-format', 'stream-json', '--stream-partial-output', '--trust'],
    cwdArgs: ['--workspace'],
    modelFlag: '--model',
    modelIds: {
      gpt: 'gpt-5.4',
  ...
  },
  {
    ...
    label: 'Gemini CLI',
    command: 'gemini',
    description: 'Google Gemini CLI over ACP with native permissions, sessions, and streamed tool activity.',
    kind: 'cli',
    protocol: 'acp',
    promptMode: 'flag',
    promptFlag: '--prompt',
