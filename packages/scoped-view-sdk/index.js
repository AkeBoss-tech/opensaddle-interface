...
export function connectScopedView({onInit, scope, timeoutMs=10000}) {
 ...
 let envelope,capabilities=[],pending,disposed=false,initialized=false
 ...
 const listener=event=>{
  ...
  if(!envelope){
   ...
   const ready=()=>{if(!disposed){send({kind:'ready'});initialized=true}}
   const failed=()=>{if(!disposed){try{send({kind:'failure'})}finally{dispose()}}}
   try{
    const setup=onInit({scope:{...message.model.scope},capabilities:[...capabilities],state:message.state})
    if(setup&&typeof setup.then==='function')Promise.resolve(setup).then(ready).catch(failed)
    else ready()
   }catch{failed()}
   return
  }
 ...
 function request(action,resource,capability,extra){
  if(disposed||!initialized)return Promise.reject(Error('View is not connected'))
 ...
 return {
  saveState(state){if(disposed||!initialized)throw Error('View is not connected');send({kind:'state',state})},
