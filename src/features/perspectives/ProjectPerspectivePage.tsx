...
function ProjectPerspectiveWorkspace({projectId,services}:{projectId:string;services:ReturnType<typeof useStore>['services']}){
 ...
 return <section className="content-page project-perspective-workspace"><header className="project-perspective-toolbar"><div><span>{projectId}</span><h1>Workspace</h1></div><label>View<select disabled={busy||!current?.layer} value={installed?.id??resolved?.perspective.id??'dialogue'} onChange={event=>void select(event.target.value)}>{[...PROJECT_PERSPECTIVES,...(current?.installed??[])].map(view=><option key={view.id} value={view.id}>{view.title}</option>)}</select></label><button disabled={busy} onClick={()=>setRefresh(value=>value+1)}>Refresh workspace</button></header>{error&&<p role="alert">{error}</p>}{busy&&!current&&<p role="status">Loading project…</p>}{current&&resolved&&<>{!installed&&resolved.fallback&&<p role="status">Your preferred view “{current.preferred}” is unavailable. Showing Dialogue; your preference is preserved.</p>}<ProjectTaskFeed key={refresh} authority={services!.journey!} projectId={projectId} connectionKey={identity??'connected'}>{model=>installed&&services?.malleableShell?<InstalledProjectView client={services.malleableShell} renderer={installed.renderer} model={model} connectionKey={identity??'connected'} stateScope={services.presentationSettings?.stateScope()} onOpenTask={id=>navigate(`/runs?run=${encodeURIComponent(id)}`)} onNewTask={()=>navigate(`/project/${encodeURIComponent(projectId)}/new-task`)}/>:<PerspectiveHost key={resolved.perspective.id} perspective={resolved.perspective} projectId={projectId} inputs={{model,onOpenTask:(id:string)=>{if(model.tasks.some(task=>task.id===id))navigate(`/runs?run=${encodeURIComponent(id)}`)},onNewTask:()=>navigate(`/project/${encodeURIComponent(projectId)}/new-task`)}}/>}</ProjectTaskFeed></>}</section>
}
