...
function ProjectPerspectiveWorkspace({projectId,services}:{projectId:string;services:ReturnType<typeof useStore>['services']}){
 ...
 const [state,setState]=useState<{services:typeof services;identity:string|undefined;model:ProjectTaskModel;preferred:string;layer?:PresentationLayer}>(),[error,setError]=useState(''),[busy,setBusy]=useState(false),[refresh,setRefresh]=useState(0)
 ...
 Promise.all([services.journey.snapshot(projectId),services.presentationSettings?.effective(projectId),services.presentationSettings?.read('user_project',projectId)]).then(([snapshot,effective,layer])=>{if(epoch===generation.current)setState({services,identity,model:projectTaskModel(snapshot,projectId),preferred:effective?.values.perspective??'dialogue',layer})}).catch(()=>{if(epoch===generation.current)setError('Could not load this project and its view preference. Check your connection and access.')}).finally(()=>{if(epoch===generation.current){setBusy(false);locked.current=false}})
 ...
 return <section className="content-page project-perspective-workspace"><header className="project-perspective-toolbar"><div><span>{projectId}</span><h1>Workspace</h1></div><label>View<select disabled={busy||!current?.layer} value={resolved?.perspective.id??'dialogue'} onChange={event=>void select(event.target.value)}>{PROJECT_PERSPECTIVES.map(view=><option key={view.id} value={view.id}>{view.title}</option>)}</select></label><button disabled={busy} onClick={()=>setRefresh(value=>value+1)}>Refresh workspace</button></header>{error&&<p role="alert">{error}</p>}{busy&&!current&&<p role="status">Loading project…</p>}{current&&resolved&&<>{resolved.fallback&&<p role="status">Your preferred view “{current.preferred}” is unavailable. Showing Dialogue; your preference is preserved.</p>}<PerspectiveHost key={resolved.perspective.id} perspective={resolved.perspective} projectId={projectId} inputs={{model:current.model,onOpenTask:(id:string)=>{if(current.model.tasks.some(task=>task.id===id))navigate(`/runs?run=${encodeURIComponent(id)}`)},onNewTask:()=>navigate(`/project/${encodeURIComponent(projectId)}/collaboration`)}}/></>}</section>
}
