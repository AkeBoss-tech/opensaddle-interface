...
export function ProjectMemoryPanel({ projectId, root, client, notify }: {
  ...
  return <div className="project-memory" aria-label="Project Memory">
    <div className="card" style={{ marginBottom: 16 }}><div className="card-header"><div><h3>Project Memory</h3><p>Authoritative KRAIL-backed context for this project.</p></div>
    ...
    {brief && <div className="card" style={{ marginBottom: 20 }}><div className="card-body"><p>{brief.summary}</p>{brief.evidence.map((item) => <blockquote key={item.id}><strong>{item.title}</strong>{item.path && <small> · {item.path}</small>}<p>{item.excerpt}</p></blockquote>)}<small>{brief.evidence.length}/{brief.maxItems} evidence items{brief.truncated ? ' · bounded result truncated' : ''}{brief.gaps.length ? ` · ${brief.gaps.length} gaps` : ''}</small></div></div>}
          ...
          {candidates.length > 0 && <><h3 className="proj-section-title">Candidate review</h3><div className="card"><div className="card-body row-list">{candidates.map((candidate) => <div className="row-item" key={candidate.candidateId}><div className="row-copy"><div className="row-title">{candidate.title}</div><div className="row-sub">{candidate.summary}</div></div><span className="status-pill yellow">{candidate.status === 'proposed' ? 'promotion proposed' : candidate.kind}</span><button className="tiny-btn" disabled={candidate.status === 'proposed' || !client.reviewMemoryCandidate || busy !== null} onClick={() => void (async () => { setBusy(candidate.candidateId); try { const reviewed = await client.reviewMemoryCandidate!(projectId, { candidateId: candidate.candidateId, decision: 'promote' }); setCandidates((current) => current.map((item) => item.candidateId === reviewed.candidateId ? reviewed : item)) } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) } finally { setBusy(null) } })()}>Promote</button><button className="tiny-btn" disabled={!client.reviewMemoryCandidate || busy !== null} onClick={() => void (async () => { setBusy(candidate.candidateId); try { const reviewed = await client.reviewMemoryCandidate!(projectId, { candidateId: candidate.candidateId, decision: 'reject' }); setCandidates((current) => current.filter((item) => item.candidateId !== reviewed.candidateId)) } catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)) } finally { setBusy(null) } })()}>Reject</button></div>)}</div></div></>}
    </>}
  </div>
}
