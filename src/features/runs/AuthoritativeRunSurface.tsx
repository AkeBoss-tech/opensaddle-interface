...
import type { FactoryCodingClient } from '../../services/factoryCoding'
...
import type { AgentResultReviewClient } from '../../services/agentResultReview'
import { AgentResultReviewPanel } from './AgentResultReviewPanel'
export type AuthoritativeRunDetail = { runId:string;projectId:string;task:string;executionInstructions?:string;status:string;workerId?:string;updatedAt?:string;cancellationRequested:boolean;canCancel:boolean;codingTask:boolean;agentResultTask?:boolean;hostedAgentResultTask?:boolean;authorizedContext?:AuthorizedContextHandle }
...
export function AuthoritativeRunSurface({authority,runId,codingResults,factoryCoding,projectId,approvalReview,connectorWriteReview,agentResultReview,hostedAgentResultReview}:{approvalReview?:RunApprovalReviewClient;connectorWriteReview?:ConnectorWriteReviewClient;agentResultReview?:AgentResultReviewClient;hostedAgentResultReview?:AgentResultReviewClient;projectId?:string;authority:AuthoritativeRunAuthority;runId:string;codingResults?:CodingResultAuthority;factoryCoding?:FactoryCodingClient}) {
  ...
  const humanReviewClient=detail?.status==='completed'?(detail.agentResultTask?agentResultReview:detail.hostedAgentResultTask?hostedAgentResultReview:undefined):undefined
  const cancel=async()=>{
  ...
  return <main className="content-page cc-page task-detail-page"><header className="page-header"><div><span className="eyebrow">Task</span><h1>{detail?.task??'Task status'}</h1></div><button className="secondary-btn" disabled={busy} onClick={()=>{packetGeneration.current++;setPacket(undefined);void refresh()}}>Refresh task status</button></header>
    ...
    {owned?.error?<p role="alert">Task unavailable: {owned.error}</p>:!detail?<p role="status">Loading the authoritative Run…</p>:<>
      ...
      {terminal(detail.status)?detail.codingTask?codingResults?<><CodingResultPanel authority={codingResults} projectId={detail.projectId} runId={detail.runId}/>{detail.status==='completed'&&factoryCoding&&<FactoryAcceptancePanel client={factoryCoding} codingResults={codingResults} projectId={detail.projectId} runId={detail.runId}/>}</>:<p>Coding result inspection is unavailable from this connection.</p>:<>{humanReviewClient?<AgentResultReviewPanel client={humanReviewClient} projectId={detail.projectId} runId={detail.runId}/>:detail.status==='completed'&&detail.hostedAgentResultTask?<p role="status">Human review of this hosted result is unavailable on this connection. Inspect recorded artifacts below.</p>:['completed','failed'].includes(detail.status)&&authority.review?<TaskResultPanel authority={authority as AuthoritativeRunAuthority & TaskResultAuthority} projectId={detail.projectId} runId={detail.runId}/>:null}{['cancelled','interrupted'].includes(detail.status)&&<p role="status">This task was cancelled or interrupted. Inspect any recorded artifacts below.</p>}<section className="cc-panel">{!humanReviewClient&&<p>Execution ended; this does not imply verification or human acceptance.</p>}<Link to={`/review?${new URLSearchParams({run:detail.runId,project:detail.projectId})}`}>Inspect result artifacts</Link></section></>:<p>The task is still active. Result verification and human review appear after execution ends.</p>}
    </>}
  </main>
}
