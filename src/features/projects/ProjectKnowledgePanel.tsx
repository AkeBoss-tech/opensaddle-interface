...
export type SourceFreshness = { state: 'unchanged' | 'changed' | 'missing' | 'unknown'; checkedCommit: string; currentDigest?: string }
export type ProjectKnowledgeCapture = { freshness?: SourceFreshness; sourceId?: string; availability?: SourceAvailability; captureId: string; path: string; commit: string; digest: string; state: 'captured' | 'reviewed' | 'unavailable'; reason?: string }
  ...
  return <section className="cc-panel" aria-label="Project knowledge">
    ...
    {!authority ? <p role="status">Project knowledge setup is unavailable from this server.</p> : <>
      ...
      {!value ? <p role="status">{current(error) ? 'Project knowledge is unavailable.' : 'Loading registered project documents…'}</p> : <>
        ...
        {value.captures.length ? value.captures.map(item => <article key={item.captureId}><strong>{item.path}</strong><p>{item.state === 'reviewed' ? 'Review recorded · current access checked on use' : item.state === 'captured' ? 'Captured · review required' : `Unavailable${item.reason ? `: ${item.reason}` : ''}`} · <code>{item.commit}</code></p><p>{item.freshness?.state === 'changed' ? 'Source content changed since capture. Review the current version before relying on this guidance.' : item.freshness?.state === 'missing' ? 'Source is absent from the checked repository revision.' : item.freshness?.state === 'unchanged' ? 'Committed source content matches this capture. This does not verify its claims.' : 'Current source freshness has not been established.'}{item.freshness && <> Checked commit <code>{item.freshness.checkedCommit}</code>.</>}</p><button disabled={busy || item.state === 'unavailable' || item.availability?.state === 'withdrawn'} onClick={() => void act(() => inspect(item))}>Inspect {item.path}</button>{item.availability && <p>Source access: {item.availability.state} · version {item.availability.revision}. Review history is retained.</p>}{item.availability && authority.setAvailability && <button disabled={busy} onClick={() => void act(() => changeAvailability(item))}>{item.availability.state === 'withdrawn' ? 'Restore source access' : 'Withdraw source access'}</button>}</article>) : <p>No project documents have been captured.</p>}
      </>}
