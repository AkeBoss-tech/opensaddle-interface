...
function ProjectPlugins({ projectId }: { projectId: string }) {
  ...
  return <section className="content-page connected-project-view">
    <header><span>{project?.name ?? projectId}</span><h1>Plugins &amp; views</h1><p>Extensions add tools, workflows, and specialized views to this project.</p></header>
    {error && <p role="alert">{error}</p>}
    {services?.malleableShell && services.presentationSettings && <ProjectViewCatalog
      key={projectId + services.presentationSettings.stateScope()}
      projectId={projectId}
      client={services.malleableShell}
      presentation={services.presentationSettings}
      installationAvailable={Boolean(services.personalRuntime)}
    />}
    {services?.extensions ? <ExtensionCatalogPanel projectId={projectId} client={services.extensions} onError={onError}/> : !services?.malleableShell && <div className="settings-card"><h2>Project extensions are not available on this runtime</h2><p>Your built-in project views are ready to use. Installed extension details require a server that supports the project extension catalog.</p><Link to="/settings">View connection settings</Link></div>}
    <section className="settings-card"><h2>Built-in project views</h2><div className="connected-view-grid"><Link to={`/project/${projectId}/overview`}>Overview<small>Repository and project profile</small></Link><Link to={`/project/${projectId}/collaboration`}>Tasks<small>Run work and inspect results</small></Link><Link to={`/project/${projectId}/knowledge`}>Knowledge<small>Sources and review history</small></Link><Link to={`/project/${projectId}/onboarding`}>Onboarding<small>Review proposed setup changes</small></Link></div></section>
  </section>
}
