...
export function ConnectedLocalProjectPage() {
  ...
  if (!project && services && (services.projectGoals || services.projectModelBudget || services.managedConnections)) return <div className="content-page connected-local-page">
    ...
    {services.projectModelBudget && <ProjectModelBudgetPanel projectId={projectId} client={services.projectModelBudget} />}
  ...
  return <div className="content-page connected-local-page">
    ...
    {services?.projectModelBudget && <ProjectModelBudgetPanel projectId={project.id} client={services.projectModelBudget} />}
...
export function ProjectProfilePanel({projectId, client}: {projectId: string; client?: LocalProjectClient}) {
  const navigate = useNavigate()
  ...
  useEffect(() => {
    ...
    void client?.onboardingState?.(projectId).then((value) => { if (!cancelled) setState(value) }).catch((reason) => { if (!cancelled) setError(reason instanceof Error ? reason.message : String(reason)) })
    return () => { cancelled = true }
  }, [projectId, client])

  if (!client?.onboardingState) return <p role="status">Project discovery and onboarding are not available on this connection. You can still set an objective and work with tasks.</p>
  return <>
