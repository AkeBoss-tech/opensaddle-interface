...
export function ConnectedJourneySurface({ authority, projectId, projectKnowledge, codingResults, mode = 'collaboration' }: { mode?: 'collaboration' | 'task'; authority: JourneyAuthority; projectId: string; projectKnowledge?: ProjectKnowledgeAuthority; codingResults?: CodingResultAuthority }) {
  ...
  if (state.authority !== authority || state.projectId !== projectId || !state.value) {
    return <main className="content-page cc-page" aria-busy="true"><h1>{mode === 'task' ? 'New task' : 'Set up collaboration'}</h1>{state.error ? <><p role="alert">{state.error}</p><button onClick={() => void load()}>Retry</button></> : <p role="status">Loading current Project access…</p>}</main>
  }
