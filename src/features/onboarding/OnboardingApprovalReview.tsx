...
export function OnboardingApprovalReview({
  ...
  return <>
    ...
    {actionable && <div className="onboarding-approval">
      <div><Icon name="shield" /><span><strong>{change.status === 'verification_failed' ? 'Verification failed on this immutable reviewed change' : 'Approve this exact digest'}</strong><small>{change.status === 'verification_failed' ? 'Retry only if the failure was transient. To change source, scope, or commands, reject and clean this worktree, then refresh and run a new recommendation.' : 'Approval runs the displayed commands, re-hashes the diff, and creates a commit only if every check passes without changing it.'}</small></span></div>
      ...
      <div className="onboarding-actions"><Button variant="danger" disabled={Boolean(busy)} loading={busy === 'reject'} onClick={onReject}>Reject & clean worktree</Button><Button disabled={Boolean(busy)} loading={busy === 'approve'} onClick={onApprove}>{change.status === 'verification_failed' ? 'Retry same verification' : 'Approve exact diff & verify'}</Button></div>
    </div>}
  </>
}
