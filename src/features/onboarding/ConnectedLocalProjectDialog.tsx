...
export function ConnectedLocalProjectDialog({ open, onClose, onRegister }: {
  ...
  return <Dialog open={open} onClose={onClose} title="Add a project" className="local-project-dialog">
    <p>Choose a folder to organize your tasks, source documents, and results in one place.</p>
    ...
    <label>Coding agent<select value={runner} onChange={(event) => setRunner(event.target.value as ProjectOnboardingRunner)}><option value="codex_cli">Codex CLI</option><option value="claude_code">Claude Code</option></select></label>
    {error && <p role="alert" className="error-text">{error}</p>}
    <div className="dialog-actions"><Button variant="secondary" onClick={onClose}>Cancel</Button><Button disabled={busy || !root.trim()} onClick={() => void submit()}>{busy ? 'Adding project…' : 'Add project'}</Button></div>
  </Dialog>
}
