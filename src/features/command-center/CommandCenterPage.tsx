...
type LoadState =
  ...
  | { kind: 'ready'; snapshot: CommandCenterSnapshot }
...
export function CommandCenterPage() {
  ...
  const load = useCallback(async () => {
    if (!services?.controlPlane.connected) {
      setState({ kind: 'unavailable', reason: 'Connect an OpenSaddle control plane to load authoritative priorities, work, and outcomes.' })
      return
    }
    if (!services.commandCenter) {
      setState({ kind: 'unavailable', reason: 'This control plane does not advertise the command_center_v1 projection.' })
      return
    }
    setState({ kind: 'loading' })
    try {
      setState({ kind: 'ready', snapshot: await services.commandCenter.get() })
    } catch (error) {
  ...
  const projectName = (projectId: string) => data.projects.find((project) => project.id === projectId)?.name ?? projectId
  ...
  return <main className="content-page cc-page">
    ...
    <CurrentObjectivePanel snapshot={snapshot} projectName={projectName} />

    <div className="cc-grid">
      <section className="cc-panel cc-attention" aria-labelledby="cc-attention-title">
        <div className="cc-section-heading"><div><span className="eyebrow">Human attention</span><h2 id="cc-attention-title">Need Akash</h2></div><strong className="cc-count">{snapshot.attentionItems.length}</strong></div>
      ...
      <section className="cc-panel" aria-labelledby="cc-runs-title">
    ...
    <section className="cc-panel" aria-labelledby="cc-projects-title">
    ...
    <section className="cc-panel" aria-labelledby="cc-outcomes-title">
