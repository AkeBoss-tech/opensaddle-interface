...
import { PersonalDevicesClient, validateResourceLimits, type AssignmentPolicy, type DeviceAssignment } from '../../services/personalDevices'
...
function AssignmentEditor({authority,deviceId,projectId,paired,initial,onChanged,teams=[]}:{authority:PersonalDevicesClient;deviceId:string;projectId:string;paired:boolean;teams?:Project[];initial?:DeviceAssignment;onChanged:(value:DeviceAssignment)=>void}) {
  ...
  useConsentClock([current])
  const expired=consentExpired(current)
  ...
  const [limitsAction,setLimitsAction]=useState<'keep'|'set'|'none'>('keep')
  const [limits,setLimits]=useState(initial?.resource_limits??{cpu_millicores:1000,memory_mib:1024,max_concurrency:1})
  let limitsValid=true;try{validateResourceLimits(limits)}catch{limitsValid=false}
  ...
  async function run(operation:()=>Promise<DeviceAssignment>) {
    ...
    try {const value=await operation();if(live.current){setCurrent(value);onChanged(value);setReview(null);setDeadlineAction('keep');setDeadline('');setLimitsAction('keep')}}catch{if(live.current)setError('Access change failed. Refresh access before trying again; another person may have changed it.')}
    finally{if(live.current){locked.current=false;setBusy(false)}}
  }
  const toggle=(values:string[],value:string)=>values.includes(value)?values.filter(item=>item!==value):[...values,value]
  const dirty=limitsAction!=='keep'||deadlineAction!=='keep' || (audience==='team_members'&&teamId!==(current?.team_id??'')) || audience!==(current?.audience??'owner_only') || JSON.stringify([...subjects].sort())!==JSON.stringify([...(current?.subjects??[])].sort()) || JSON.stringify([...sources].sort())!==JSON.stringify([...(current?.source_ids??[])].sort()) || JSON.stringify([...adapters].sort())!==JSON.stringify([...(current?.adapter_ids??[])].sort())
  return <section className="device-assignment-editor"><p role="status">{current ? `Access policy: ${current.state}` : 'No project access configured'}</p>{current?.state==='accepted' && <p>{!expired&&current.consent_allows_requester?'Your task-use consent is active.':'This policy does not currently authorize your tasks.'} A connected, configured worker is also required.</p>}{current&&<><ConsentDeadline item={current}/><ResourceLimitSummary item={current}/></>} {error && <p role="alert">{error}</p>}
    {context && <fieldset disabled={busy || !paired}><legend>Owner’s task-use policy</legend><label>Who may use this device?<select value={audience} onChange={event=>{setAudience(event.target.value as AssignmentPolicy['audience']);setSubjects([]);setReview(null)}}><option value="owner_only">Only me</option>{(teams.length>0||current?.team_id)&&<option value="team_members">Members of a named team</option>}<option value="selected_members">Selected project members</option><option value="project_members">All current project members</option></select></label>
      ...
      <label>Resource limits<select aria-label="Resource limits" value={limitsAction} onChange={event=>{setLimitsAction(event.target.value as 'keep'|'set'|'none');setReview(null)}}><option value="keep">Keep current limits</option><option value="set">Set limits</option><option value="none">Remove limits</option></select></label>
      {limitsAction==='set'&&<fieldset><legend>Shared project allowance on this device</legend>{([['cpu_millicores','CPU (millicores)',1000000],['memory_mib','Memory (MiB)',67108864],['max_concurrency','Concurrent demand',10000]] as const).map(([key,label,max])=><label key={key}>{label}<input aria-label={label} type="number" min={1} max={max} step={1} value={Number.isNaN(limits[key])?'':limits[key]} onChange={event=>{setLimits(previous=>({...previous,[key]:event.target.value===''?NaN:Number(event.target.value)}));setReview(null)}}/></label>)}<p>These admission limits apply across all workers on this device for this project. They do not cap operating-system usage.</p>{!limitsValid&&<p>Enter positive whole numbers within the allowed ranges.</p>}</fieldset>}
      {limitsAction==='none'&&<p>Removing owner limits requires fresh project acceptance. Project limits still apply.</p>}
      ...
      <button disabled={(limitsAction==='set'&&!limitsValid)||!deadlineValid || !sources.length || !adapters.length || (audience==='selected_members'&&!subjects.length) || (audience==='team_members'&&!teamId)} onClick={()=>void run(()=>authority.proposeAssignment(deviceId,projectId,{expected_revision:current?.revision??0,...(limitsAction==='keep'?{}:{resource_limits:limitsAction==='none'?null:limits}),...(deadlineAction==='keep'?{}:{expires_at:deadlineAction==='none'?null:new Date(deadlineTime).toISOString()}),audience,...(audience==='team_members'?{team_id:teamId}:{}),subjects,source_ids:sources,adapter_ids:adapters}))}>Propose access</button>
    </fieldset>}
    {!paired && <p>Pair this device before proposing access.</p>}
    {current?.state==='proposed' && context?.canManage && !dirty && <button disabled={busy||expired} onClick={()=>{if(!consentExpired(current))setReview('accept')}}>Review project acceptance</button>}
    ...
    {review && current && <div className="device-access-review"><ResourceLimitSummary item={current}/><ConsentDeadline item={current}/><p>{review==='accept'?'Accept the saved owner policy for this project?':'Revoke task-use consent for this project?'} This does not change access for other projects.</p><button disabled={busy||(review==='accept'&&expired)} onClick={()=>{if(review==='accept'&&consentExpired(current))return;void run(()=>review==='accept'?authority.decideAssignment(deviceId,projectId,current.revision,true):authority.revokeAssignment(deviceId,projectId,current.revision))}}>{review==='accept'?'Accept policy':'Confirm revocation'}</button><button disabled={busy} onClick={()=>setReview(null)}>Cancel</button></div>}
  </section>
}
