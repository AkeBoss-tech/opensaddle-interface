...
export function DeviceActivity({authority,deviceId,projects=[]}:{authority:PersonalDevicesClient;deviceId:string;projects?:{id:string;name:string}[]}){
 ...
 useEffect(()=>{let live=true,timer:ReturnType<typeof setTimeout>|undefined;setLoaded(undefined);setError('')
  if(!open)return
  async function read(){try{const value=await authority.activity(deviceId);if(live)setLoaded({authority,identity,deviceId,value})}catch{if(live){setLoaded(undefined);setError('Device activity is unavailable. Check your connection and access.')}}finally{if(live)timer=setTimeout(()=>void read(),5000)}}
