...
export function ProjectTaskFeed({authority, projectId, connectionKey, children}: {
  authority: Pick<JourneyAuthority, 'snapshot'>
  projectId: string
  connectionKey: string
  children: (model: ProjectTaskModel) => ReactNode
}) {
  const [value, setValue] = useState<{authority: typeof authority; projectId: string; connectionKey: string; model: ProjectTaskModel}>()
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        const model = projectTaskModel(await authority.snapshot(projectId), projectId)
        if (stopped || expired) return
        setValue(previous => previous?.authority === authority && previous.projectId === projectId && previous.connectionKey === connectionKey && JSON.stringify(previous.model) === JSON.stringify(model)
          ? previous : {authority, projectId, connectionKey, model})
        setError('')
      } catch {
        if (!stopped) {
          setValue(undefined)
  ...
  }, [authority, projectId, connectionKey])
  const current = value?.authority === authority && value.projectId === projectId && value.connectionKey === connectionKey ? value.model : undefined
