export type PresentationScope='user'|'project'|'user_project'
...
export class PresentationSettingsClient {
  ...
  private path(scope:PresentationScope,projectId?:string){if(scope==='user')return '/api/v2/settings/presentation';if(!projectId)throw Error('Project required');return `/api/v2/projects/${encodeURIComponent(projectId)}/settings/presentation/${scope}`}
...
function layer(value:any,scope:PresentationScope,projectId?:string):PresentationLayer {if(typeof value.can_write!=='boolean'||value.scope!==scope||value.project_id!==(scope==='user'?null:projectId)||!Number.isSafeInteger(value.revision)||value.revision<0)throw Error('Invalid settings scope');return {...value,values:validateValues(value.values)}}
