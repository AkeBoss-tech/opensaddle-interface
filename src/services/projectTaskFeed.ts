...
export interface ProjectTaskFeedAuthority {read(projectId:string):Promise<ProjectTaskModel>}
export class ProjectTaskFeedClient implements ProjectTaskFeedAuthority {
 ...
 async read(projectId:string){
  ...
  const page=async(cursor:string)=>{const response=await fetch(`${this.base}/api/v2/projects/${encodeURIComponent(projectId)}/task-feed?${new URLSearchParams({after:cursor,limit:'100'})}`,{cache:'no-store',signal:AbortSignal.timeout(15000),headers:{'X-OpenSaddle-User':subject,...(this.token?{Authorization:`Bearer ${this.token}`}:{})}});if(!response.ok)throw Error('Project task feed unavailable');const value=await response.json();if(subject!==this.user()||value.schema_version!=='opensaddle.project-task-feed.v1'||value.project_id!==projectId||!Array.isArray(value.items)||value.items.length>100||(value.next_cursor!==null&&typeof value.next_cursor!=='string'))throw Error('Invalid Project task feed');return value}
