...
export interface ApplicationRendererDescriptor{application_id:string;instance_id:string;entry_file:string;content_digest:string;size:number;media_type:'text/html; profile=opensaddle-renderer-fragment.v1; charset=utf-8';package_ref:{package_id:string;version:string;manifest_digest:string};input_schema:Record<string,unknown>;output_schema:Record<string,unknown>;state_schema_version:number;sandbox_policy:{scripts:true;same_origin:false;network_isolation:'unavailable';navigation_containment:'host_observed_only'};authority:'core';execution_trust:'trusted_signed_publisher'}
export interface ApplicationRendererCandidate{application_id:string;title:string;package_id:string;package_version:string;manifest_digest:string;content_digest:string;size_bytes:number;state_schema_version:number;state_compatibility:null|{accepts_from_versions:number[]};available:{available:boolean;reason:string|null};enablement:null|{status:'enabled'|'disabled';version:string;revision:number};activation:{desired:boolean;observed_health:'unavailable';receipt:null};environment_application:EnvironmentApplicationDefinition|null}
