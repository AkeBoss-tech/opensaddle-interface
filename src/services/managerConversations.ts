...
export class ManagerConversationsClient implements ManagerConversationsAuthority {
 ...
 async list(){const items:ManagerConversation[]=[],seen=new Set<string>(),cursors=new Set<string>();let cursor='';do{const page=await this.request('?limit=100'+(cursor?'&cursor='+encodeURIComponent(cursor):''));if(!Array.isArray(page.items)||page.items.length>100||(page.next_cursor!==null&&typeof page.next_cursor!=='string'))throw Error('Invalid manager conversation page');for(const item of page.items){const value=conversation(item);if(seen.has(value.conversation_id))throw Error('Conversation list changed; reload it');seen.add(value.conversation_id);items.push(value)}if(page.next_cursor&&(cursors.has(page.next_cursor)||cursors.size>=20||items.length>=1000))throw Error('Conversation list exceeds supported page limits');cursor=page.next_cursor??'';cursors.add(cursor)}while(cursor);return items}
 async open(id:string){const value=conversation(await this.request('/'+encodeURIComponent(id)));if(value.conversation_id!==id)throw Error('Conversation identity mismatch');const messages:ManagerMessage[]=[],cursors=new Set<string>();let cursor='',sequence=0;do{const page=await this.request('/'+encodeURIComponent(id)+'/messages?limit=100'+(cursor?'&cursor='+encodeURIComponent(cursor):''));if(page.provider_execution!==false||page.conversation_version!==value.version||!Array.isArray(page.items)||page.items.length>100||(page.next_cursor!==null&&typeof page.next_cursor!=='string'))throw Error('Conversation changed; reload it');for(const message of page.items){if(message.thread_id!==id||typeof message.message_id!=='string'||!integer(message.sequence)||message.sequence<=sequence||message.role!=='user'||typeof message.content!=='string'||Array.from(message.content).length>20000||!validScope(message.payload?.manager_scope)||message.payload.provider_status!=='not_started')throw Error('Invalid manager message');sequence=message.sequence;messages.push(message)}cursor=page.next_cursor??'';if(cursor&&(cursors.has(cursor)||messages.length>=1000))throw Error('Message history exceeds supported page limits');cursors.add(cursor)}while(cursor);const latest=conversation(await this.request('/'+encodeURIComponent(id)));if(latest.conversation_id!==id||latest.version!==value.version||JSON.stringify(latest.scope)!==JSON.stringify(value.scope))throw Error('Conversation changed; reload it');return {conversation:value,messages}}
 async create(title:string,projects:string[]){const user=this.user();const intent=await this.intent('create',{title,projects});if(user!==this.user())throw Error('Manager account changed');const value=conversation(await this.request('','POST',{request_id:intent.id,title,project_ids:projects}));if(value.title!==title||JSON.stringify(value.scope.project_ids)!==JSON.stringify(projects))throw Error('Created conversation scope mismatch');intent.clear();return value}
 async append(value:ManagerConversation,content:string){const user=this.user();const intent=await this.intent('message',{id:value.conversation_id,scope:value.scope,content});if(user!==this.user())throw Error('Manager account changed');const result=await this.request('/'+encodeURIComponent(value.conversation_id)+'/messages','POST',{request_id:intent.id,expected_version:value.version,expected_scope_revision:value.scope.revision,content});if(result.provider_execution!==false||result.message?.thread_id!==value.conversation_id||result.message?.content!==content||result.message?.role!=='user'||result.message?.client_message_id!==intent.id||result.message?.message_id!=='msg_'+intent.id||!integer(result.conversation_version)||result.message?.payload?.provider_status!=='not_started'||JSON.stringify(result.message?.payload?.manager_scope)!==JSON.stringify(value.scope))throw Error('Saved message receipt mismatch');intent.clear()}
 async scope(value:ManagerConversation,projects:string[]){const result=conversation(await this.request('/'+encodeURIComponent(value.conversation_id)+'/scope','PUT',{expected_version:value.version,expected_scope_revision:value.scope.revision,project_ids:projects}));if(result.conversation_id!==value.conversation_id||JSON.stringify(result.scope.project_ids)!==JSON.stringify(projects))throw Error('Saved scope mismatch')}
}
