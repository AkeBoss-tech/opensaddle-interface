...
export class PersonalDevicesClient {
  ...
  async proposeAssignment(deviceId: string, projectId: string, policy: AssignmentPolicy) {
    if(policy.resource_limits!=null)validateResourceLimits(policy.resource_limits)
    ...
    if(Object.hasOwn(policy,'resource_limits')&&(value.resource_limits===undefined||(policy.resource_limits===null?value.resource_limits!==null:!value.resource_limits||Object.keys(policy.resource_limits!).some(key=>policy.resource_limits![key as keyof ResourceLimits]!==value.resource_limits![key as keyof ResourceLimits]))))throw Error('Resource limits could not be confirmed. Refresh access.')
    return value
  }
...
export interface ResourceLimits {cpu_millicores:number;memory_mib:number;max_concurrency:number}
export function validateResourceLimits(raw:unknown):ResourceLimits {
 const value=record(raw),bounds={cpu_millicores:1000000,memory_mib:67108864,max_concurrency:10000}
 if(Object.keys(value).length!==3||Object.entries(bounds).some(([key,max])=>!Number.isSafeInteger(value[key])||Number(value[key])<1||Number(value[key])>max))throw Error('Invalid resource limits')
 return value as unknown as ResourceLimits
}
export interface AssignmentPolicy {resource_limits?:ResourceLimits|null;expires_at?:string|null;expected_revision:number;audience:'owner_only'|'selected_members'|'project_members'|'team_members';team_id?:string|null;subjects:string[];source_ids:string[];adapter_ids:string[]}
...
function assignment(raw:unknown,deviceId:string,projectId?:string):DeviceAssignment {
  ...
  if(value.resource_limits!=null)validateResourceLimits(value.resource_limits)
