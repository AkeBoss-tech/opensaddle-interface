...
test('AGENT-SETUP-DESKTOP-1: renderer passes task idempotency only on the exact task route',async()=>{
 const originalFetch=globalThis.fetch,priorWindow=globalThis.window,seen:unknown[]=[]
 ;(globalThis as unknown as{window:unknown}).window={opensaddle:{personalRuntimeRequest:async(value:unknown)=>{seen.push(value);return{status:200,contentType:'application/json',bodyBase64:'e30='}}}}
 globalThis.fetch=async()=>{throw Error('network fetch must not run')}
 try{
  const{installPersonalRuntimeTransport}=await import(`./personalRuntimeTransport?task=${Date.now()}`)
  installPersonalRuntimeTransport({baseUrl:'http://127.0.0.1:8766',installationId:'install',projectId:'project'})
  await fetch('http://127.0.0.1:8766/api/v2/agents/ptc_one/tasks',{method:'POST',headers:{'Idempotency-Key':'task-key-1'},body:'{}'})
  assert.deepEqual(seen[0],{path:'/api/v2/agents/ptc_one/tasks',method:'POST',idempotencyKey:'task-key-1',expectedBaseUrl:'http://127.0.0.1:8766',expectedInstallationId:'install',expectedProjectId:'project',body:'{}'})
  await fetch('http://127.0.0.1:8766/api/v2/agent-proposals/agp_one/publish',{method:'POST',headers:{'Idempotency-Key':'other-key'},body:'{}'})
  assert.equal((seen[1] as Record<string,unknown>).idempotencyKey,undefined)
