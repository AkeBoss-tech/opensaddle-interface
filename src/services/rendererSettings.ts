...
export type RendererSettingsScope='project'|'user_project'
...
export interface RendererSettingsSnapshot {contract:RendererSettingsContract;layers:Array<{scope:RendererSettingsScope;revision:number;can_write:boolean;values:ApplicationState}>;effective:{values:ApplicationState;provenance:Record<string,string>}}
export interface RendererSettingsAuthority {identity():string;read(project:string,renderer:ApplicationRendererDescriptor):Promise<RendererSettingsSnapshot>;replace(project:string,renderer:ApplicationRendererDescriptor,scope:RendererSettingsScope,revision:number,values:ApplicationState):Promise<RendererSettingsSnapshot>}
...
export class RendererSettingsClient implements RendererSettingsAuthority{
 ...
 private async request(project:string,renderer:ApplicationRendererDescriptor,scope?:RendererSettingsScope,revision?:number,values?:ApplicationState){
  ...
  const response=await fetch(`${this.base}/api/v2/projects/${encodeURIComponent(project)}/application-renderers/${encodeURIComponent(renderer.application_id)}/settings${scope?'/'+scope:''}?${query}`,{method:scope?'PUT':'GET',cache:'no-store',signal:AbortSignal.timeout(15000),headers:{'X-OpenSaddle-User':subject,...(this.token?{Authorization:'Bearer '+this.token}:{}),...(scope?{'Content-Type':'application/json'}:{})},body:scope?JSON.stringify({expected_revision:revision,values}):undefined})
  ...
  const result=await response.json(),scopes=['project','user_project'].filter(item=>contract.scopes.includes(item))
 ...
 replace(project:string,renderer:ApplicationRendererDescriptor,scope:RendererSettingsScope,revision:number,values:ApplicationState){return this.request(project,renderer,scope,revision,values)}
}
