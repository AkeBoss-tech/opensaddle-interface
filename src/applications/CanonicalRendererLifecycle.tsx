...
export function CanonicalRendererLifecycle({ client, resource, projection, connectionKey, applicationId = 'review-evidence', instanceId = 'review-main' }: { client: MalleableShellClient; resource: ExactArtifactRef; projection: ApplicationProjection; connectionKey: string; applicationId?: string; instanceId?: string }) {
  ...
  const load = useCallback(async (previous?: Ready['previous']) => {
    const current = ++generation.current
    if (!previous) setState({ kind: 'loading' })
    ...
    try {
      ...
      const allowStateTransfer = Boolean(previous && selected?.state_compatibility?.accepts_from_versions.includes(previous.renderer.state_schema_version))
      setState({ kind: 'ready', client, environment, renderer, candidates: eligible, previous, hostStatus: 'loading', allowStateTransfer })
    } catch (reason) {
