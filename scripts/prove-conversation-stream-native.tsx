...
const [state,receipt,authorization,selectedAdapter='codex-app-server']=process.argv.slice(2)
assert.ok(selectedAdapter==='codex-app-server'||selectedAdapter==='claude-code-stream-json')
...
let ready=false
for(let i=0;i<30&&!ready;i++){const snapshot=await journey.snapshot(m.project_id);ready=Boolean(snapshot.nativeAdapters?.some(a=>a.sourceId===m.source_id&&a.adapterId===selectedAdapter&&a.ready));if(!ready)await pause(2000)}
...
const task=await client.dispatch(opened.conversation,message.message_id,m.project_id,m.source_id,selectedAdapter)
...
try{
 ...
 writeFileSync(receipt,JSON.stringify({schema_version:'opensaddle.native-conversation-stream-proof.v1',adapter_id:selectedAdapter,project_id:m.project_id,conversation_id:conversation.conversation_id,message_id:message.message_id,run_id:task.run_id,first_observed_status:firstObservedStatus,rendered_updates:updates,first_preview:firstPreview,last_preview_characters:lastPreview.length,output,checks:['actual local '+selectedAdapter+' task','real Core SSE and Interface conversation client','mounted saved-message preview rendered while Run running','multiple rendered text updates','preview prefixes match digest-checked final native result'],limits:['Mounted React evidence; not a browser screenshot or keyboard acceptance','Only the selected adapter is exercised by this receipt']},null,2)+'\n')
