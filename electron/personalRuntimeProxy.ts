...
const agentResultReview = /^\/api\/v2\/runs\/[A-Za-z0-9._~-]+\/agent-result\/review$/
const projectMemberRemoval = /^\/api\/v2\/projects\/[A-Za-z0-9._~-]+\/members\/remove$/
const managedConnectionRoute = /^\/api\/v2\/projects\/([A-Za-z0-9._~-]+)\/connector-connections(?:\/([A-Za-z0-9._~-]+)\/revoke)?$/
...
const historyRoute = /^\/api\/v2\/projects\/([A-Za-z0-9._~-]+)\/history\/(sessions(?:\/(his_[a-f0-9]{32})(\/visibility)?)?|search)$/
function historyRequest(path:string,method:string):string|undefined{
 if(!path.startsWith('/api/v2/')||path.startsWith('//'))return undefined
 const url=new URL(path,'http://localhost'),match=historyRoute.exec(url.pathname)
 if(url.origin!=='http://localhost'||url.pathname+url.search!==path)return undefined
 if(!match)return undefined
 const params=url.searchParams,keys=[...params.keys()]
 if(match[2]==='sessions'&&method==='GET'&&keys.join(',')==='offset,limit'&&/^(0|[1-9][0-9]{0,5})$/.test(params.get('offset')??'')&&params.get('limit')==='100')return match[1]
 if(match[2]==='search'&&method==='GET'&&keys.join(',')==='q,limit'&&(params.get('q')??'').length>=2&&(params.get('q')??'').length<=200&&params.get('limit')==='50')return match[1]
 if(match[3]&&match[4]===undefined&&method==='GET'&&keys.length===0)return match[1]
 if(match[3]&&match[4]==='/visibility'&&method==='PUT'&&keys.length===0)return match[1]
 return undefined
}
...
const allowed:Array<[PersonalRuntimeProxyRequest['method'],RegExp]>=[
 ...
 ['GET',/^\/api\/v2\/projects\/[A-Za-z0-9._~-]+\/connector-connections$/],
 ['POST',managedConnectionRoute],
 ...
 ['GET',agentResultReview],
 ['POST',agentResultReview],
...
function validateConnectionBody(raw:unknown,revoke:boolean){const body=parseAgentBody(raw,'connection');if(revoke){if(Object.keys(body).join(',')!=='expected_revision'||!Number.isSafeInteger(body.expected_revision)||Number(body.expected_revision)<1)throw Error('Personal runtime connection body is invalid');return}if(Object.keys(body).sort().join(',')!=='api_key,connector,display_name,secret_ref'||['connector','secret_ref','display_name'].some(key=>typeof body[key]!=='string'||!String(body[key]).trim()||String(body[key]).length>(key==='display_name'?120:200)||/[\u0000-\u001f\u007f]/.test(String(body[key])))||typeof body.api_key!=='string'||!body.api_key||body.api_key.length>4096)throw Error('Personal runtime connection body is invalid')}
