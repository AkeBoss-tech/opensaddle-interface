...
## Browser helper (preview)

`packages/scoped-view-sdk/index.js` exports `connectScopedView`. It is a local,
private package, not a published registry dependency. Bundle it into the signed
HTML fragment with your build tool; opaque renderer frames cannot import remote
modules. Sign the final generated bytes, including the helper.
...
const view = connectScopedView({
  scope: 'user',
  onInit({state, scope}) {
    document.querySelector('#title').textContent = `Devices for ${scope.id}`;
...
document.querySelector('#refresh').onclick = async () => {
  try {
    ...
    document.querySelector('#result').textContent =
      page.items.map(device => device.display_name).join('\n');
  } catch {
    document.querySelector('#result').textContent = 'Devices unavailable. Retry.';
  }
};
document.querySelector('#filter').onchange = event =>
  view.saveState({filter: event.target.value});
...
checks parent identity and the complete envelope, correlates resource responses,
