...
Final local evidence: focused operations tests passed; full repository tests 377/377 passed (313 workspace plus 64 feature/service); typecheck passed; production build passed with the existing large-chunk advisory; `git diff --check` passed. Isolated Chrome headless rendered the authenticated Core 8903 operations, participant inbox, and exact result journeys without using the user's browser profile. At 390 x 844 the compact Home, Start, Work, Operations, and Settings navigation remains visible, occupies the bottom 51 pixels, and no longer clips content; automation clicked the four requested destinations and then clicked the exact inbox result. The result remained under `/opensaddle-interface/review` with the exact Run and invocation query. Fresh desktop and mobile screenshots are saved under `docs/agents/evidence/`. The earlier reconnect failure was an automation selector error: the all-settings page retains inactive panel controls in the DOM, and the script populated the General panel's inactive password field. Selecting the visible Connection panel before typing connected successfully; no product connection fault was found. This does not claim a physical touch device or production deployment.

The first result screenshot still exposed horizontal overflow from intrinsic card, select, and long-reference widths. The Review workspace now constrains the page, cards, unavailable-state children, and fields to their grid columns while wrapping long evidence. Fresh Review and Evidence perspective screenshots show readable complete copy, a bounded artifact selector, exact resource lineage, and durable invocation evidence. Chrome measured both `document.documentElement.scrollWidth` and `document.body.scrollWidth` at 390 pixels for a 390-pixel viewport.
...
### 2026-09-07 — PostgreSQL Command Center and Operations browser integration
...
- Shell selection now follows successful authenticated v2 capability negotiation, including additive `capability_mode` values such as `postgres_conformance`; unknown server mode labels render as `Server` rather than falling through to demo/company workspace UI.
- A v2-only connection no longer instantiates legacy workspace or thread clients, so it does not call unsupported `/api/workspace`, show mock Corporate Base identities, or report a false save failure. Project registration remains hidden unless the server advertises the local-project client.
- Verified connection profiles persist in tab-scoped session storage, including the bearer needed for an authenticated hard reload; choosing demo mode clears that profile. The PG8904 owner connection remained on 8904 across an uncached reload.
- Fresh clean-shell browser evidence: `docs/agents/evidence/command-center-postgres-8904-clean-desktop-2026-09-07.png`, `docs/agents/evidence/command-center-postgres-8904-clean-mobile-2026-09-07.png`, `docs/agents/evidence/operations-postgres-8904-clean-desktop-2026-09-07.png`, and `docs/agents/evidence/operations-postgres-8904-clean-mobile-2026-09-07.png`. Both 390px routes measured `scrollWidth === innerWidth`; no token is visible.

### 2026-09-07 — exact artifact handoff across registered applications

- Added the independently registered `opensaddle.artifact.evidence.v1` application and `/artifact-evidence` route. Review exposes an explicit Open with action carrying exact Project, Run, artifact, digest, command version/digest, origin application, and optional durable invocation.
- The evidence destination independently rereads the authorized artifact list, current Project command descriptors, and durable invocation. Missing/revoked resources, changed digests, stale descriptors, substituted invocation identity, route changes, and client changes fail closed without retained protected content.
- Returning to Review preserves the exact artifact selection and durable invocation. Live Core8903 network observation recorded zero POST requests across Review → Evidence → Review, proving view changes did not dispatch or rerun the command.
- Browser proof used the real completed fixture artifact and invocation. Desktop and 390px destination views showed the second application identity, origin, exact resource, descriptor, receipt, and invocation; the narrow view measured `scrollWidth === innerWidth` and retained compact navigation. Evidence: `docs/agents/evidence/cross-app-review-origin-core8903-desktop-2026-09-07.png`, `docs/agents/evidence/cross-app-evidence-destination-core8903-desktop-2026-09-07.png`, `docs/agents/evidence/cross-app-evidence-destination-core8903-mobile-2026-09-07.png`.

### 2026-09-07 — authenticated artifact report bytes

- The evidence application now reads the exact artifact content endpoint after resource and command reauthorization, incrementally caps responses at 256 KiB, verifies SHA-256 before decoding, requires valid UTF-8, and distinguishes unavailable bytes, oversized content, and integrity denial without retaining a prior report.
- Untrusted report text uses an inert React renderer with no HTML injection. The live fixture's literal `<script>` probe remained visible text and its `javascript:` Markdown link remained inert; DOM inspection found zero script elements and zero unsafe anchors.
- The report leads with its title and bounded body, followed by authenticated source/freshness and durable result cards. Exact resource, descriptor, invocation, and receipt IDs moved into a closed technical inspector.
- Core8903 v7 returned 279 real immutable bytes whose digest matched the artifact record. The rendered source `fixture://participant-proof/report-v1` and explicit no-KRAIL/no-verification statement remained intact. A separate metadata-only artifact produced `artifact_bytes_unavailable` with no prior report content. The existing desktop/mobile evidence paths were refreshed with this real-byte journey.

### 2026-09-07 — personal application presentation

- The independently registered evidence application now binds Core application `review.evidence` instance `review-main`. Its report title, body, source, freshness, and durable result render before application provenance and exact identifiers in the closed technical inspector. A leading Markdown heading promoted to the page title is omitted once; later headings remain intact.
- Authenticated personal settings read the server's effective instance and offer density and document/split/focus presentation. **Check changes** calls the personal preview route; **Save for me** sends exact personal revision CAS plus the previewed shared environment revision/digest. **Restore previous** targets the immutable parent revision. Shared defaults remain separately inspectable and are never edited by these controls. Conflicts and denials are explicit.
- Pending checks, saves, and restores synchronously lock both selectors and actions. A deferred response cannot authorize a changed draft, and a shared-environment CAS conflict leaves the applied presentation unchanged. Project-substituted personal responses fail closed.
- The artifact reader was also checked against the reported ten-chunk, missing-length reproduction. It stops after the first over-limit read, cancels the reader, and never appends the over-limit chunk; both queued and pull-driven cancellation regressions pass.
- Real Core8903 v8 browser proof used two different authenticated members. Member one rendered its revision-1 compact/focus preference at 390px after the shared application v1→v2 upgrade. Member two rendered comfortable/split, saved compact/document after a checked change, retained that setting across reload, and restored its prior revision to revision 3. Member one remained unchanged. The exact 279-byte report stayed visible, DOM inspection found zero scripts and unsafe links, and the 390px document measured `scrollWidth === innerWidth`. No command invocation was sent and the fixture's queued participant message remained queued.
- Evidence: `docs/agents/evidence/cross-app-evidence-destination-core8903-mobile-2026-09-07.png` (member one compact/focus), `docs/agents/evidence/application-personalization-saved-core8903-desktop-2026-09-07.png` (member two saved compact/document), and `docs/agents/evidence/application-personalization-core8903-desktop-2026-09-07.png` (member two restored comfortable/split). No token is visible.
- Validation: 315 workspace tests, 89 feature/service tests, typecheck, production build, and `git diff --check` pass. Vite retains the existing large-chunk advisory. This proves stored and rendered presentation through the authenticated API; it does not claim extension hot reload or a general replacement SDK.

### 2026-09-07 — Command Center connection isolation (#36)

- Test-first evidence used this same mounted suite with the generation, cleanup, and render-time fences removed from the production surface: replacement client A settled after client B and visibly replaced **Current B** with **Private A**; an older overlapping refresh visibly replaced **Latest** with **Older**. The unmodified repaired surface passes all five mounted cases.
- Command Center loading now lives in a mountable surface whose every success and error is bound to both the owning service-bundle identity and the exact command-center client. A monotonically increasing request generation invalidates older requests on disconnect, client replacement, every Refresh/Retry, effect cleanup, and unmount. Cancellation is not required for correctness.
- Render-time checks suppress a ready/error/unavailable state captured from a different bundle or client before the replacement effect completes. Disconnect and missing-capability props render their current unavailable state directly, so protected Project, Goal, approval, Run, and outcome text cannot remain visible during revocation.
- Mounted deferred-promise coverage proves disconnect then old success; client B success then client A success/error; two overlapping refreshes where the older success/error settles last; StrictMode replay; unmount/reopen; and a ready snapshot followed by an exact client replacement under the same outer identity.
- Validation: 315 workspace tests, 94 feature/service tests, typecheck, production build, and `git diff --check` pass. Vite retains the existing large-chunk advisory.

### 2026-09-07 — executable application boundary (development and desktop foundation)

- Added exact renderer manifest and byte validation for the Core `opensaddle-renderer-fragment.v1` profile, including its 128 KiB bound, MIME, package/version/manifest/content digests, declared message schemas, UTF-8, fragment structure, sandbox policy, and incremental response cancellation. The authenticated client now exposes exact Project renderer catalog/content routes without passing a token or service client into application code.
- The web host binds every message to the current iframe `WindowProxy`, nonce, generation, instance, connection, and package ref; accepts only bounded `filter`/`note` state; rate-limits messages; clears state on authority changes; synchronously removes an old frame during replacement; aborts pending reads on replacement; and exposes host recovery for invalid, hung, or unexpectedly navigating frames. A development-only report annotator demonstrates compatible v1/v2 state preservation. Fixture controls are absent from production builds and normal evidence flows.
- Real Chrome proved `connect-src 'none'` does block `fetch`, a forged parent-window message cannot alter renderer state, and a navigation attempt leaves the host route and triggers recovery. It also proved that a script-enabled sandboxed iframe can issue its own navigation request before recovery. Therefore the web implementation explicitly does **not** claim arbitrary untrusted code, network isolation, or canonical activation. Core's corrected 8905 contract truthfully advertises trusted signed publisher execution with network isolation unavailable.
- Added a desktop isolation foundation using a dedicated Electron `WebContentsView`, unique in-memory session partition, no Node integration, context isolation, sandboxing, a minimal per-instance transferred `MessagePort`, main/subframe/redirect/new-window/download denial, pre-request cancellation, sender/main-frame and instance identity fences, bounded message validation/rate limits, separate renderer-process check, and a watchdog installed before loading. The main process receives no service client or token. This adapter is not yet wired to the normal product surface, and a packaged Electron hung-renderer journey remains required before claiming complete desktop activation.
- Ready-state screenshots: `docs/agents/evidence/executable-renderer-fixture-desktop-2026-09-07.png` and `docs/agents/evidence/executable-renderer-fixture-mobile-2026-09-07.png`. Both show the useful exact-resource report renderer; the 390px view retains the compact shell and readable controls. The screenshots contain no token.
