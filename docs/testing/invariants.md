...
- **DESKTOP-PERSONAL-RESUME-1** — After failed startup adoption, the desktop offers an explicit restart only when bounded private retained state binds the same installation and Project, the owned socket refuses connection, and the prior loopback endpoint is absent. Partial, mismatched, symlinked, unreadable, recovery-required, or uncertain live state blocks setup instead of minting a replacement identity. A current or ambiguous daemon lock blocks; an exact same-host dead-PID lock is left to Core for final stale-lock recovery. The user reselects an available coding agent and limits, and the restart request and returned handoff must preserve the exact installation and Project; Core still enforces its lock, registered repository, and Run reconciliation. Account or connection replacement invalidates the candidate. `test/personalRuntimeResume.test.ts` exercises the real filesystem/socket/lock boundary; mounted form and store tests check the explicit request and identity fencing. Restart creates no new task, though Core may claim already admitted queued work after its checks; this does not prove crash-safe recovery.
...
- **PROJECT-RUN-AUDIT-PAGE-1** — When Core advertises the finite Run event-page contract, active and ended task inspection reads one bounded page on request, advances the exact stored-event cursor only on explicit Load more or Refresh, and distinguishes more stored events from future active-Run events. The desktop proxy admits only the exact bounded read. A denied or malformed page clears previously rendered protected digests; older Core is limited to ended-Run SSE and clearly rejects active-Run audit inspection.
...
The rendered connected sidebar marks the active task route, keeps Home/Work/Operations/Settings reachable, and does not invent Project identity or offer registration when no registration service exists. A mounted surface failure leaves sibling shell navigation present and retry restores content. src/features/shell/ConnectedNavigation.test.tsx uses the real StoreProvider, sidebar and SurfaceErrorBoundary; only browser storage and CSS loading are adapted for the Node runner. It replaces inline-sidebar and exact boundary-key source assertions. Existing useProductSurface.test.tsx and connectedProductSurface.test.ts cover authenticated capability selection and reconnect identity; CommandCenterPage.mounted.test.tsx and recorded desktop Home journeys cover the Command Center component. These do not collectively prove every App route composition. This is stale-test consolidation with no production change, not a pre-fix production regression claim. The obsolete five-button mobile assertion remains failing until the drawer receives direct visual verification.
...
In a packaged app, the bundled Core executable is available only when the exact KRAIL runtime manifest, pinned interpreter, dependency records, KRAIL commands, and Core launcher validate together. A stray Core launcher cannot start the managed backend after the bundle is damaged or incomplete. Manifest, digest records and launchers must resolve inside their declared resource root, even when a path is a symlink; symlinks remaining inside the bundle are allowed. Commands must resolve to regular executable files, not directories. Ambient development executable, resource-root and KRAIL-command overrides cannot replace the validated packaged pair for either the managed sidecar or a commissioned personal runtime; a missing bundle remains explicitly unavailable. Development mode retains its configured-executable and command overrides. The resource-root tests reproduced prior acceptance of both an external launcher and a directory launcher; the commissioned subprocess test reproduced inheritance of a foreign KRAIL command before the pinned environment was passed to spawn. Owning boundaries: `test/krailRuntimeBundle.test.ts` and `test/personalRuntimeProcess.test.ts`.
